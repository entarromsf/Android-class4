...
public class Clock extends View {
    ...
    private void drawNeedles(final Canvas canvas) {
        ...
        paintSecond.setStrokeWidth(5);    //秒针刻度线
        ...
        int lenHour = mCenterX - (int) (mWidth * 0.01f)-350;
        int lenMin = mCenterX - (int) (mWidth * 0.01f)-250;
        int lenSec = mCenterX - (int) (mWidth * 0.01f)-150;

        cul = 180 + 360 + 90 - hourdregrees;
        if(cul > 360)
            cul -= 360;
        int stophX = (int) (mCenterX + lenHour * Math.cos(Math.toRadians(cul)));
        int stophY = (int) (mCenterX - lenHour * Math.sin(Math.toRadians(cul)));
        canvas.drawLine(mCenterX, mCenterY, stophX, stophY, paintHour);

        cul = 180 + 360 + 90 - minudegrees;
        if(cul > 360)
            cul -= 360;
        int stopmX = (int) (mCenterX + lenMin * Math.cos(Math.toRadians(cul)));
        int stopmY = (int) (mCenterX - lenMin * Math.sin(Math.toRadians(cul)));
        canvas.drawLine(mCenterX, mCenterY, stopmX, stopmY, paintMinute);

        cul = 180 + 360 + 90 - senconddegree;
        if(cul > 360)
            cul -= 360;
        int stopsX = (int) (mCenterX + lenSec * Math.cos(Math.toRadians(cul)));
        int stopsY = (int) (mCenterX - lenSec * Math.sin(Math.toRadians(cul)));
        canvas.drawLine(mCenterX, mCenterY, stopsX, stopsY, paintSecond);
    }
